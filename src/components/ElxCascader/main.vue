<template>
  <span class="el-cascader" :class="[
    ...
    <el-input ref="input" :readonly="!filterable" :placeholder="currentLabels.length ? undefined : placeholder" v-model="inputValue" @input="debouncedInputChange" @focus="handleFocus" @blur="handleBlur" :validate-event="false" :size="size" :disabled="cascaderDisabled">
...
<script>
import Vue from "vue";
import ElCascaderMenu from "element-ui-x/packages/cascader/src/menu";
import ElInput from "element-ui-x/packages/input";
import Popper from "element-ui-x/src/utils/vue-popper";
...
import { generateId } from "element-ui-x/src/utils/util";
...
export default {
    ...
    currentObject() {
      ...
      this.currentValue.forEach(value => {
        const targetOption =
          ...
          options.filter(option => option[this.valueKey] === value)[0];
        if (targetOption) {
          currentObject.push(targetOption);
          options = targetOption[this.childrenKey];
        }
      });
  ...
  watch: {
    ...
    currentLabels(value) {
      const inputLabel = this.showAllLevels
        ? value.join("/")
  ...
  methods: {
    ...
    handlePick(value, close = true) {
      ...
      this.$emit("input", value, this.currentLabels);
      this.$emit("change", value, this.currentLabels, this.currentObject);
    ...
    handleInputChange(value) {
      ...
      let filteredFlatOptions = flatOptions.filter(optionsStack => {
        return optionsStack.some(option =>
          new RegExp(value, "i").test(option[this.labelKey])
        );
      });
