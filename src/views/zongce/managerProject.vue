  ...
  methods: {
    stateFormatter(r, c, v, i) {
      for (var j = 0; j < this.projectStateList.length; j++) {
        if (this.projectStateList[j].value == v) {
          return this.projectStateList[j].label;
        }
      }
    },
    showProspectus(row) {
    ...
    createProject() {
      this.$router.push({
        path: "/project/createAppraiseProject",
        query: {
          serType: "201809"
        }
      });
    ...
    ...mapActions({
      ...
      queryAppraiseProject: store.namespace + "/queryAppraiseProject"
    }),
    getProjectState() {
      this.getProjectStateList({}).then(response => {
    ...
    getData() {
      ...
      //查询数据的方法
      this.queryAppraiseProject(requestData).then(response => {
