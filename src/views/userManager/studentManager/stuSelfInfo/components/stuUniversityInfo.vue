<template>
	<div>
		<el-form size="mini" :model="formStore.formData.data" ref='universityForm' :rules='formStore.formData.rules' label-width="100px">
...
export default {
	  ...
	  watch: {
	    ...
	    'loadOrg.orgCode': function(val) {
	      ...
	      GUtils.post(reqPath.queryCurrentOrgListByOrgCode, { 'orgCode': val }, function(data) {
	        f_org_code.pushAll(data.resBody)
	      })
    }
	    //			active() {
	    //				var universityInfo = {
	    //					"universityInfo": this.formStore.formData.data
	    //				};
	    //				this.$emit("form1", universityInfo);
	    //			}
	  },
	  methods: {
	    submit: function() {
	      GUtils.post(dataPath.submitStuSelfUniversity, this.formStore.formData.data, function(data) {
	        alert('提交成功')
      })
	    }
	  }
