...
export default {
	  ...
	  methods: {
	    submitUpdate: function() {
	      ...
	      GUtils.post(dataPath.updateStuUniversityInfoSelf, this.formStore.formData.data,
	        function(data) {
	          delete that.formStore.formData.data.managerNodeCode
	          alert('修改成功')
          // 加载要修改的源数据
          GUtils.post(dataPath.getStudentUniversityInfoById, {
	            'id': data.resBody.id
	          }, function(data2) {
	            console.log('updateStuUniversityInfoData', data2)
	            formDataStore.pushData(
	              data2.resBody.baseData[0]
	            )
            formDataStore.pushData({
