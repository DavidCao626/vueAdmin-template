<template>
	<div>
		<el-form size="mini" :model="formStore.formData.data" ref='form1' disabled :rules='formStore.formData.rules' label-width="80px">
			<el-form-item label="头像">
				<img v-if="formStore.formData.data.headPortrait != ''" :src="formStore.formData.data.headPortrait"  alt="加载失败" title="点击修改" height="80px" width="80px">
			</el-form-item>
			...
			<el-form-item label="住宿类型">
			    ...
			    <el-option
			      v-for="(item,index) in adTypeData[0]"
			      :key="index"
			      :label="item.dict_desc"
			      :value="item.dict_key">
			    </el-option>
			  </el-select>
			</el-form-item>
...
GUtils.post(dataPath.getDictByDictNames, { 'dicts': dicts }, function(data) {
...
export default {
	  ...
	  mounted: function() {
	    GUtils.post(dataPath.getStudentPersonalInfoById, { 'id': this.bid }, function(data) {
