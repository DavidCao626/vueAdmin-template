	...
	<div id="bodyBox">
		<gc-hcontainer :component-store="pageHCon">
			...
			<div style="margin-right:120px;margin-top:5px;margin-left:10px">
              ...
              <el-form-item label="可访问用户状态" prop="role_val">
							  <gc-s-sel :component-store="f_req_state"></gc-s-sel>
							</el-form-item>
							<el-form-item label="可访问权值" prop="role_val">
              ...
              <el-form-item label="可访问用户状态" prop="role_val">
							  <gc-s-sel :component-store="n_f_req_state"></gc-s-sel>
							</el-form-item>
							<el-form-item label="权值配置" prop="role_val">
...
var f_req_state = GStoreFactory.buildSmallSelectStore(formDataStore.data, 'req_state')
...
var n_f_req_state = GStoreFactory.buildSmallSelectStore(nchild_formDataStore.data, 'req_state')
...
GUtils.post(reqPath.queryDecortor, {
  'decoras': ['menuTypeDecorator', 'iconDecorator','roleSessionStateDecorator']
}, function(data) {
  ...
  GUtils.assembleComponent(data.resBody, 'roleSessionState', f_req_state, {
    'name': 'title',
    'val': 'value'
  })
  ...
  GUtils.assembleComponent(data.resBody, 'iconStore', n_f_icon, {
   ...
   GUtils.assembleComponent(data.resBody, 'roleSessionState', n_f_req_state, {
    'name': 'title',
    'val': 'value'
  })
})
...
nroleCheck.getResolver().callStore(function(callback) {
  GUtils.post(
    reqPath.queryDefaultEnablePrivilegeState,{},
    function(data) {
      nchild_formDataStore.req_state=data.resBody.sessionState;
      callback.call(nroleCheck.getResolver(), data.resBody.typeAndPrivilege);
    }
  );
});
...
export default {
 ...
 //   })
    ...
    treeStore.addConf('nodeClick', function(data, node, tree) {
      if (node.data.root_no && node.data.root_no == '000') {
        ...
        GUtils.post(reqPath.queryMenu, {
          ...
          index.formDataStore.data.req_state= result.sessionState
  ...
  methods: {
    addMenu: function() {
      console.log(nchild_formDataStore.data);
  ...
  watch: {
    ...
    'formDataStore.data.req_state':function(val){
      console.log(val);
      var that=this;
  roleCheck.getResolver().callStore(function(callback) {
  GUtils.post(
    reqPath.queryEnablePrivilegeState,{ sessionStateLong: val },
    function(data) {
      callback.call(roleCheck.getResolver(), data.resBody);
    }
  );
});
    },
     'nchild_formDataStore.data.req_state':function(val){
      console.log(val);
    nroleCheck.getResolver().callStore(function(callback) {
    GUtils.post(
    reqPath.queryEnablePrivilegeState,{ sessionStateLong: val },
    function(data) {
      callback.call(nroleCheck.getResolver(), data.resBody);
